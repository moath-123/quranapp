...
object QuranMetaDataHelper {

    private val MADANI_SURAS = setOf(
        2, 3, 4, 5, 8, 9, 13, 22, 24, 33,
        47, 48, 49, 55, 57, 58, 59, 60,
        61, 62, 63, 64, 65, 66, 76, 98,
        99, 110
    )

    fun revelationTypeFor(suraNumber: Int): String {
        return if (MADANI_SURAS.contains(suraNumber)) "Madani" else "Makki"
    }

    // ---------------- Juz (Chapter/Part) Names ----------------
    ...
    fun juzNameFor(juzNumber: Int): JuzName {
        return JUZ_NAMES[juzNumber] ?: JuzName("Juz' $juzNumber", "الجزء $juzNumber")
    }
